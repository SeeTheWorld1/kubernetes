// If -m is used, remember that nsenter operates on the workdir of the target id
...
// Command create the command with a default background context
func (c *Config) Command(program string, args ...string) (*exec.Cmd, error) {
	cmd, err := c.buildCommand(context.Background())
